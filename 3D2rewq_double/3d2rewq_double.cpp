...
int main(int argc, char **argv)
{
    ...
    double dtx,dtz,dtxz,dr1,dr2,dtx4,dtz4,dtxz4;
    double xmax,px,sx;
    double vvp2,drd1,drd2,vvs2,tempux2,tempuy2,tempuz2,tempvx2,tempvy2,tempvz2,
    ...
    u       = (double*)malloc(sizeof(double)*nz*ny*nx);
    v       = (double*)malloc(sizeof(double)*nz*ny*nx);
    w       = (double*)malloc(sizeof(double)*nz*ny*nx);
    up      = (double*)malloc(sizeof(double)*nz*ny*nx);
    up1     = (double*)malloc(sizeof(double)*nz*ny*nx);
    up2     = (double*)malloc(sizeof(double)*nz*ny*nx);
    vp      = (double*)malloc(sizeof(double)*nz*ny*nx);
    vp1     = (double*)malloc(sizeof(double)*nz*ny*nx);
    vp2     = (double*)malloc(sizeof(double)*nz*ny*nx);
    wp      = (double*)malloc(sizeof(double)*nz*ny*nx);
    wp1     = (double*)malloc(sizeof(double)*nz*ny*nx);
    wp2     = (double*)malloc(sizeof(double)*nz*ny*nx);
    us      = (double*)malloc(sizeof(double)*nz*ny*nx);
    us1     = (double*)malloc(sizeof(double)*nz*ny*nx);
    us2     = (double*)malloc(sizeof(double)*nz*ny*nx);
    vs      = (double*)malloc(sizeof(double)*nz*ny*nx);
    vs1     = (double*)malloc(sizeof(double)*nz*ny*nx);
    vs2     = (double*)malloc(sizeof(double)*nz*ny*nx);
    ws      = (double*)malloc(sizeof(double)*nz*ny*nx);
    ws1     = (double*)malloc(sizeof(double)*nz*ny*nx);
    ws2     = (double*)malloc(sizeof(double)*nz*ny*nx);
    vpp     = (double*)malloc(sizeof(double)*nz*ny*nx);
    density = (double*)malloc(sizeof(double)*nz*ny*nx);
    vss     = (double*)malloc(sizeof(double)*nz*ny*nx);
	...
	fclose(flog);
        ...
        for(i=0;i<nz;i++)
            for(j=0;j<ny;j++)
                for(k=0;k<nx;k++)
                {
                    u[i*ny*nx+j*nx+k]=0.0;
                    v[i*ny*nx+j*nx+k]=0.0;
                    w[i*ny*nx+j*nx+k]=0.0;
                    up[i*ny*nx+j*nx+k]=0.0;
                    up1[i*ny*nx+j*nx+k]=0.0;
                    up2[i*ny*nx+j*nx+k]=0.0;
                    vp[i*ny*nx+j*nx+k]=0.0;
                    vp1[i*ny*nx+j*nx+k]=0.0;
                    vp2[i*ny*nx+j*nx+k]=0.0;
                    wp[i*ny*nx+j*nx+k]=0.0;
                    wp1[i*ny*nx+j*nx+k]=0.0;
                    wp2[i*ny*nx+j*nx+k]=0.0;
                    us[i*ny*nx+j*nx+k]=0.0;
                    us1[i*ny*nx+j*nx+k]=0.0;
                    us2[i*ny*nx+j*nx+k]=0.0;
                    vs[i*ny*nx+j*nx+k]=0.0;
                    vs1[i*ny*nx+j*nx+k]=0.0;
                    vs2[i*ny*nx+j*nx+k]=0.0;
                    ws[i*ny*nx+j*nx+k]=0.0;
                    ws1[i*ny*nx+j*nx+k]=0.0;
                    ws2[i*ny*nx+j*nx+k]=0.0;
                }//for(k=0;k<nx;k++) end
        for(l=1;l<=lt;l++)
        {
            xmax=l*dt*velmax;
        ...
        fwrite(up+169*ny*nx,sizeof(double),ny*nx,fout);
